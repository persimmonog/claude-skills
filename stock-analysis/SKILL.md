...
stock-analysis/
  └── <股票名称>/                     # 以公司中文名命名（来自 longbridge company）
    ...
    └── source_docs/               # 一手原始文档（文件名含公司名+发布时间YYYYMMDD）
        ├── 年报_公司名_20260409.txt   # 美股：SEC EDGAR 10-K全文
        ├── 季报_公司名_20260502.txt   # 美股：SEC EDGAR 10-Q全文
        ├── 委托书_公司名_20260409.txt # 美股：SEC EDGAR DEF 14A全文
...
**以下 7 项各服务不同的分析目的，缺失任何一项都会在报告对应部分留下空白。7 项全部标记 ✅/🚫 才是第 2a 层的结束信号。**

**关键：年报和季报均无法获取 → 停止分析**，告知用户无法继续。

**进度追踪表（所有 ⬜ → ✅/🚫 之前，禁止进入 Phase 2b）：**

| # | 必选 | 下载文件 | 服务于 | 状态 |
|---|------|------|--------|------|
| 1 | 全部市场 | 年报 | Step 1 生意本质 + Step 4 财务质量 | ⬜ |
| 2 | 全部市场 | 季报/中报 | 最新季度经营变化 + Step 4 | ⬜ |
| 3 | 美股/A股 | 电话会 Q&A | Step 1 管理层讨论 + Step 3 竞争优势 | ⬜ |
| 4 | 上市≤5年 | 招股书 | Step 1 行业概况 + Step 6 增长驱动 | ⬜ |
| 5 | 全部市场 | 同行年报(≥2家) | Step 3 竞争格局 + Step 7 相对估值 | ⬜ |
| 6 | 全部市场 | 行业研报(≥1份) | Step 2 行业空间 + Step 6 | ⬜ |
| 7 | 全部市场 | 宏观与政策 | Step 6 宏观环境 + Step 8 风险 | ⬜ |

**执行规则：**
- **1-7项必须先完成实体文件下载，放在 source_docs/ 中，才能进入 Phase 2b。不允许仅凭 WebSearch 摘要或者其他材料里有相似内容就标记完成——必须有 PDF 或 txt 文件落盘。**
- 每完成一项立刻更新状态，然后直接进入下一项
- **#3 完成后，强制输出：**「第 2a 层进度：✅#1 ✅#2 ✅#3 → 剩余 #4 #5 #6 #7 待执行，继续下载」—— 这段输出本身是对你自己的提醒
- #4 若上市>5年则标记 🚫 并注明原因，**但仍需继续 #5 #6 #7**
- **禁止在 #1-#7 全部处理完之前启动 Phase 2b**，哪怕你觉得数据已经足够
- 全部完成后输出进度表终态
...
3. 框架文件中每个 Step 下方的所有问题，**都必须逐一回答**。

**写作质量标准：**

每个问题的回答要达到「让没看过原始数据的人也能完整理解」的程度。具体标准：

- **禁止作答方式：** 一句话结论、只给数字不给背景、只描述趋势不解释原因。"营收增长 20%"不可接受——要说清楚从多少到多少、靠什么业务驱动、增长质量如何。
- **回答深度要求：** 每个问题的回答至少包含：① 具体数据和事实（来自 critical_data.md 或 data/ 目录）② 数据背后的原因或逻辑（为什么会是这个数字？）③ 与前期的对比或趋势判断（变好还是变坏？拐点出现了吗？）
- **数字要有上下文：** 提到任何数字时，同时给出同比/环比、绝对值和变化幅度。不能只说"毛利率 45%"，要写"毛利率 45%，同比提升 3 个百分点，主要受原材料成本下降和产品结构升级驱动"。
- **结论要有证据链：** 每个判断后面紧跟支撑它的具体数据或年报原文——"管理层声称 X"→ 立即用毛利率/ROIC/复购率数据验证或证伪。

这些标准适用于全部 9 个 Step 的每一个问题，无一例外。
